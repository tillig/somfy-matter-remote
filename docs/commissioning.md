...
The dashboard's status table reports the installed firmware version and, next to it, whether a newer release exists. The comparison runs in your browser against the public GitHub API rather than on the device, so the ESP32 makes no outbound calls and needs no TLS stack or certificate bundle. If the browser has no internet access, or the GitHub API rate limit (60 requests per hour per address) is reached, the row simply shows the installed version with no update note; it never claims a state it could not confirm. A build from source between releases reports a descriptive version such as `v1.0.0-3-gabc1234`, and the check stays quiet because such a build is not comparable to a release.
